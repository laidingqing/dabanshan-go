...
import (
	...
	"github.com/laidingqing/dabanshan/episodes/service"
...
func main() {
	...
	config.ParseCmdParams(config.DefaultCmdLine{
		HostName:         "localhost",
		Port:             4200,
		ServiceName:      config.ServiceEpisodeName,
		RegistryLocation: "http://127.0.0.1:2379",
	})

	// Set up the core logger
	log.SetOutput(&lumberjack.Logger{
		Filename:   "./logs/episodes_service.log",
	...
	log.Printf("starting episode service at %d", config.Service.Port)
	s := grpc.NewServer()
	pb.RegisterEpisodeServiceServer(s, &service.RPCEpisodeServer{})
	s.Serve(lis)
}
